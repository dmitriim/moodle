        ...
        if ($rs = $extdb->Execute($sql)) {
            if (!$rs->EOF) {
                while ($fields = $rs->FetchRow()) {
                    ...
                    if ($startdate) {
                        // Start date filed is not found in the table. Let's stop syncing to avoid unexpected course start date.
                        if (!key_exists($startdatelowercased, $fields)) {
                            $trace->output('error: configured course start date not found: ' . json_encode($fields), 1);
                            continue;
                        }
                    ...
                    if ($enddate) {
                        // End date filed is not found in the table. Let's stop syncing to avoid unexpected course end date.
                        if (!key_exists($enddatelowercased, $fields)) {
                            $trace->output('error: configured course end date not found: ' . json_encode($fields), 1);
                            continue;
                        }
